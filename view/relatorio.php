...
<html lang="pt-br">
    ...
    <body>
        <?php 
            ...
            $pdf->SetY(100); $pdf->SetX(5);
            $pdf->SetFont('OpenSans','',12);
            $pdf->SetFillColor(255,255,255);
            $pdf->SetTextColor(0,0,0);
            $pdf->Cell(50,35, utf8_decode('Descrição do defeito:'),'B',0,"R","true");
            $pdf->MultiCell(90, 5, utf8_decode($arraySelectOS['descricao_defeito']),'','J',false);

            //Descrições das Ordens de serviço.
            $pdf->SetY(135); $pdf->SetX(5);
            $pdf->SetFont('OpenSans','',12);
            $pdf->SetFillColor(240,248,255);
            $pdf->SetTextColor(0,0,0);
            $pdf->Cell(50,35, utf8_decode('Descrição do reparo:'),'B',0,"R","true");
            $pdf->MultiCell(90, 5, utf8_decode($arraySelectOS['descricao_reparo']),'','J',false);

            //Aqui eu exibo a data e hora atual.
            $pdf->SetFont('OpenSans','',8);
            $pdf->SetFillColor(255,255,255);
            $data_atual = date('d/m/Y - H:i:s');
            $pdf->SetY(170); $pdf->SetX(88);
            $pdf->Cell(55,10, utf8_decode("Documento gerado em: $data_atual"),0,0,"C","false");
